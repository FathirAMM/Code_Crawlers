  ...
  return (
    <div className={classnames(styles.config, 'flex-c-sb flex-column')}>
      ...
      <Panel className={styles.panel} title="Global OpenAI Config">
          ...
          <Panel.Item icon="model" title="model" desc="existing llm">
            <Select options={modelOptions} value={openai.model} onChange={val => setModel({ model: val })} placeholder="Choose models" />
          </Panel.Item>
          <Panel.Item icon="model" title="custom model" desc="Custom llm.">
            <Input style={{border:"5%"}} type='text' autoComplete="new-text" value={openai.model} onChange={val => setModel({ model: val })} placeholder="ft-chatgpt" />
          </Panel.Item>
          <Panel.Item icon="model" title="Database name" desc="Custom vector databese.">
            <Select options={dbOptions} value={openai.database} onChange={val => setModel({ database: val })} placeholder="Choose models" />
          </Panel.Item>
